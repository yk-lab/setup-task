...
/**
 * Per-request timeout and response-body size cap (NFR-1). Bound how long a
 * fetch may hang and how much a (potentially hijacked) trusted host may stream,
 * so neither can wedge the action or exhaust memory. Both are generous: the API
 * pages and `task_checksums.txt` sit far below the byte cap.
 */
