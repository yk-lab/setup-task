...
- **Proxy (`proxy.ts`)**: `configureProxyFromEnv()` runs first in `run()` and, when a proxy env var is set, installs undici's `EnvHttpProxyAgent` as the global dispatcher so built-in `fetch` honours `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` — Node's fetch ignores them otherwise (tool-cache already does). Verified by the `proxy` self-test job (tinyproxy). No-op without a proxy env.
